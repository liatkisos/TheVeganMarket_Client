...
export class OrderFormComponent implements OnInit {
  ...
  checkCreditCard() {
    ...
    if (isNaN(creditCardValue)) {
      ...
      this.creditCardErrorMsg = "please enter 16 numerical digits only.";
    }
    else {
  ...
  sendOrderForm() {
    ...
    if (!this.orderForm.controls.city.valid || !this.orderForm.controls.street.valid || !this.orderForm.controls.deliveryDate.valid) {
      ...
      this.msg = "Please fill out all inputs fields!";

    }
    else {

      if (!this.orderForm.controls.creditCard.valid) {
        console.log(this.orderForm.controls.creditCard);
  ...
  getInvoice() {
    ...
    TextInvioce += `The Vegan Market Invoice  ${today.toLocaleDateString()} ${today.toLocaleTimeString()} \r\n\r\n`;
    ...
    TextInvioce += "Thank you for your purchase and hope to see you back soon!";

    var blob = new Blob([TextInvioce], { type: "text/plain;charset=utf-8" });
    
    saveAs(blob, "The Vegan Market Invoice.txt");
